# **Custom Diffusion 环境配置完整指南 **

PS： 大型文件都删除了，需要根据指南自己下载。
...
5.  #### **创建并激活 Conda 环境 (`ldm`)**:
    ...
    *   **Conda 环境创建问题排查 (来自您的笔记)**:
        ...
        *   **`clip` 和 `taming-transformers` 下载或安装失败**:
            ...
            ```
            如果这些自动下载失败 (例如由于网络问题或git相关错误)，您可以按照您的笔记进行手动安装：
            1.  **手动下载源码**:
        ...
        *   **`conda env create` 失败的其他原因**:
            ...
            *   尝试解决 `environment.yaml` 中指出的具体包版本冲突，一定注意完全按照`environment.yaml` 里的要求安装环境。
...
2.  #### **项目代码修改**：
    ...
    *   **修改 `scripts/finetune_gen.sh` (使用自定义正则化图像)**：
        如果您希望==**使用自己准备的正则化图像**==，而不是让脚本自动生成，请注释掉调用 `sample.py` 生成正则化图像的部分。如果让脚本自动生成就不需要注释。
    ...
    *   **主要改动说明：**
        ...
        这个修改后的片段应该能解决您遇到的 `AttributeError`，并更稳健地处理不同类型的 `gpus` 配置。

### **二、数据准备**

1.  #### **概念图像 (Concept Images)**：
    
    *   准备一组代表您要微调的新概念的图像（例如，特定风格的猫的图片）。
    *   将这些图像放置在一个文件夹中。
    *   **您的示例路径**：`data/cat/`
...
2.  #### **正则化图像 (Regularization Images)**：
    ...
    *   这些图像用于防止模型遗忘原始概念。
    *   如果您选择使用自定义的正则化图像（已修改 `finetune_gen.sh`）：
        *   准备与概念类别相同但风格泛化的图像（例如，普通的猫的照片）。
        *   将这些图像放置在 `finetune_gen.sh` 脚本参数 `${ARRAY[2]}/samples/` 所指向的目录。
        *   **您的示例路径**：`gen_reg/samples_cat/samples/` (注意，脚本中 `${ARRAY[2]}` 是 `gen_reg/samples_cat`，然后 `train.py` 会查找其下的 `samples` 子目录)。
...
2.  #### **运行微调脚本**：
    ...
    **您的示例日志文件夹**：`logs/2025-05-10T15-52-41_cat-sdv4/`
...
3.  #### **输出**：
    ...
    **您的 Delta 权重文件路径**：`logs/2025-05-10T15-52-41_cat-sdv4/checkpoints/delta_epoch=last.ckpt`

### **六、使用微调模型生成图像**

现在，结合原始预训练模型、提取的 Delta 权重和您的特殊 Token 来生成图像。
...
通过以上步骤，您应该能够成功地对单个概念进行微调，并使用该微调模型生成符合您要求的图像。
...
1.  #### **项目与环境**：
    ...
    *   **项目根目录**：`/data/coding/upload-data/data/custom-diffusion/` (所有命令均在此目录下执行)。
    *   **Conda 环境**：`ldm`。激活命令：`conda activate ldm`。
    *   **操作系统/Shell**: 根据命令格式判断为 Linux 环境。
